...
export default async function Home({ searchParams }: { searchParams: Promise<{ view?: string }> }) {
  ...
  return <main className="app-shell">
    ...
    <div className="content"><header><div><p className="eyebrow">{titles[view].eyebrow}</p><h1>{titles[view].title}</h1></div><span className="secure"><Shield size={15}/> Secure workspace</span></header>
      ...
      {view === "groupme" && isOperationsManager ? <><GroupMeConnectionManager connections={groupMeConnections}/><GroupMeInbox connections={groupMeConnections} initialSubmissions={groupMeSubmissions} teams={teams.map(team=>({id:team.id,name:team.name}))} broadcasts={groupMeBroadcasts}/></> : null}
