...
export async function POST(request:Request){
  try{
    ...
    if(action==="add_note"){
      ...
      const parentVisible=(body.parentVisible===true||text(body,"parentVisible")==="on")&&["development","strength","improvement","attendance","general"].includes(category);
      const{error}=await supabase.from("player_development_notes").insert({player_id:playerId,team_id:player.team_id,coach_id:profile?.coach_id??null,author_id:user.id,category,attendance_status:category==="attendance"?attendanceStatus:null,contact_method:["parent_contact","payment_discussion"].includes(category)?contactMethod:null,contact_outcome:["parent_contact","payment_discussion"].includes(category)?contactOutcome:null,manager_only:category==="payment_discussion",parent_visible:parentVisible,activity_date:text(body,"activityDate")||new Date().toISOString().slice(0,10),note,follow_up_needed:body.followUp===true||text(body,"followUp")==="on"});if(error)throw error;
