...
import { ChevronDown, FileDown, Plus, Save, Trash2, UserPlus, WalletCards, X } from "lucide-react";

type TeamItem = { team: string; amount: number };
type StaffOption = { id: string; name: string; role: string; isCoach: boolean };
export type PayrollRow = { id?: string; coach_id?: string | null; staff_name: string; role: string | null; hourly_rate: number; skills_hours: number; additional_hours: number; team_stipend: number; manager_pay: number; bonus: number; tracked_time?:boolean;tracked_entry_count?:number; team_items?: TeamItem[]; extra_pay_note?: string | null; bonus_note?: string | null };
...
export function PayrollPlanner({ initialRows, currentMonth, staffOptions }: { initialRows: PayrollRow[]; currentMonth: string; staffOptions: StaffOption[] }) {
  ...
  function downloadSummary(){const quote=(value:unknown)=>`"${String(value??"").replaceAll('"','""')}"`;const headers=["Staff","Role","Hourly Rate","Skills Hours","Other Approved Hours","Team Stipends","Manager Pay","Bonus","Gross Pay","Extra Pay Note","Bonus Note"];const lines=[headers.map(quote).join(","),...rows.map(row=>{const totals=calc(row);return[row.staff_name,row.role??"",n(row.hourly_rate).toFixed(2),n(row.skills_hours).toFixed(2),n(row.additional_hours).toFixed(2),totals.first.toFixed(2),n(row.manager_pay).toFixed(2),n(row.bonus).toFixed(2),totals.gross.toFixed(2),row.extra_pay_note??"",row.bonus_note??""].map(quote).join(",")})];const blob=new Blob([lines.join("\r\n")],{type:"text/csv;charset=utf-8"}),url=URL.createObjectURL(blob),link=document.createElement("a");link.href=url;link.download=`invictus-payroll-${currentMonth.slice(0,7)}.csv`;link.click();URL.revokeObjectURL(url)}
  ...
  return <section className="payroll-planner" id="payroll">
    ...
    <div className="payroll-table-wrap"><table className="payroll-table"><thead><tr><th>Staff</th><th>Role</th><th>$/hour</th><th>Skills hours</th><th>Extra hours</th><th>Team stipends</th><th>Manager pay</th><th>Bonus</th><th>Gross</th><th></th></tr></thead><tbody>{rows.map((row, index) => <tr key={row.id ?? `new-${index}`}>
      ...
      {(["hourly_rate", "skills_hours", "additional_hours"] as Array<keyof PayrollRow>).map((key) => {const tracked=Boolean(row.tracked_time)&&["skills_hours","additional_hours"].includes(key);return <td key={key}><input className={tracked?"tracked-payroll-input":""} type="number" min="0" step="0.01" value={String(row[key] ?? 0)} readOnly={tracked} title={tracked?"Calculated from approved coach time":undefined} onFocus={(event) => !tracked&&event.currentTarget.value === "0" && event.currentTarget.select()} onChange={(event) => !tracked&&set(index, key, event.target.value)}/>{tracked?<small className="tracked-time-label">Approved time</small>:null}</td>})}
    ...
    <div className="payroll-actions"><button className="secondary" onClick={() => setRows((current) => [...current, emptyRow()])}><Plus size={15}/>Add payroll row</button><button className="secondary" onClick={() => setShowStaffForm(true)}><UserPlus size={15}/>Add new staff</button><button className="secondary" onClick={save} disabled={saving}><Save size={15}/>{saving ? "Saving..." : "Save payroll plan"}</button><button className="secondary" onClick={downloadSummary}><FileDown size={15}/>Download Gusto summary</button></div>
    {message ? <p className="plan-message">{message}</p> : null}<p className="muted payroll-note">Approved coach time flows into the hour columns automatically. Download the CSV for review and entry into Gusto; this app does not issue payments.</p>
  </section>;
}
