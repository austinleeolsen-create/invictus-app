...
export function TeamOperations({ teams, coaches, seasons, canEdit, canCreateSeason }: { teams:TeamSummary[]; coaches:CoachSummary[]; seasons:SeasonOption[]; canEdit:boolean; canCreateSeason:boolean }) {
  ...
  useEffect(()=>{if(panel)requestAnimationFrame(()=>panelRef.current?.scrollIntoView({behavior:"smooth",block:"start"}))},[panel,team,coach]);
  ...
  async function deleteTeam(){if(!team)return;if(team.playerCount>0){setMessage("Move the players off this team before deleting it.");return}if(!window.confirm(`Delete ${team.name}? This removes the team and its coach assignments. This cannot be undone.`))return;setSaving(true);setMessage("");try{await request({action:"delete_team",id:team.id});window.location.reload()}catch(error){setMessage(error instanceof Error?error.message:"Unable to delete team.");setSaving(false)}}
  return <section className="team-ops" id="teams">
    ...
    {panel?<div className="operation-panel" ref={panelRef}><div className="panel-title"><h3>{panel==="team"?`${team?"Edit":"Add"} team`:panel==="coach"?`${coach?"Edit":"Add"} coach`:panel==="assign"?"Assign coach":panel==="bulk"?"Bulk add 2026–2027 teams":"Add season"}</h3><button aria-label="Close form" onClick={()=>setPanel(null)}><X/></button></div>
      {panel==="team"?<><form onSubmit={submit}><input type="hidden" name="action" value="save_team"/><input type="hidden" name="id" value={team?.id??""}/><label>Team name<input name="name" defaultValue={team?.name??""} required/></label><label>Age group<input name="ageGroup" defaultValue={team?.ageGroup??""}/></label><label>Season<select name="seasonId" value={seasonId} onChange={event=>{if(event.target.value==="__add__"){setInlineSeason(true);setSeasonId("")}else setSeasonId(event.target.value)}} required><option value="">Choose season...</option>{seasonOptions.map(season=><option value={season.id} key={season.id}>{season.name}</option>)}{canCreateSeason?<option value="__add__">+ Add a new season</option>:null}</select></label><button disabled={saving||!seasonId}>{saving?"Saving...":"Save changes"}</button></form>{team?<div className="team-delete-row"><div><strong>Delete this team</strong><span>{team.playerCount?`${team.playerCount} player${team.playerCount===1?" is":"s are"} assigned and must be moved first.`:"No players are assigned. Coach assignments will also be removed."}</span></div><button type="button" onClick={deleteTeam} disabled={saving||team.playerCount>0}>Delete team</button></div>:null}{inlineSeason?<form className="inline-season-form" onSubmit={createInlineSeason}><div className="inline-form-heading"><strong>Add a season without losing this team</strong><button type="button" onClick={()=>setInlineSeason(false)}><X size={15}/></button></div><label>Season name<input name="name" placeholder="2026–2027" required/></label><label>Start date<input name="startDate" type="date"/></label><label>End date<input name="endDate" type="date"/></label><button disabled={saving}>{saving?"Adding...":"Add and select season"}</button></form>:null}</>:null}
