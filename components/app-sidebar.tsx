...
export function AppSidebar({ view, name, role, showFinancials, isOperationsManager, groupMePending=0, courtPending=0, children }: { view: string; name: string; role: string; showFinancials: boolean; isOperationsManager: boolean; groupMePending?:number; courtPending?:number; children: ReactNode }) {
  const [open, setOpen] = useState(false);
  const group = (label: string, items: NavItem[]) => <details className="nav-group" open={items.some(item => item.key === view)}><summary><span>{label}</span><ChevronDown size={14}/></summary><div>{items.map(item => <a key={item.key} className={view === item.key ? "active" : ""} href={`/?view=${item.key}`} onClick={() => setOpen(false)}>{item.icon}{item.label}{item.badge?<b className="nav-alert-badge">{item.badge>99?"99+":item.badge}</b>:null}</a>)}</div></details>;
